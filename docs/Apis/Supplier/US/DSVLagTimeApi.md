# Walmart\Apis\Supplier\US\DSVLagTimeApi  
...
```php
getLagTime($shipNode, $accept, $wMPARTNERID, $sku, $gtin): \Walmart\Models\Supplier\US\DSVLagTime\LagTimeResponse
```
Lag Time
...
try {
    $result = $api->getLagTime($shipNode, $accept, $wMPARTNERID, $sku, $gtin);
    print_r($result);
} catch (Exception $e) {
...
```php
updateLagTimeBulk($feedType, $shipNode, $accept, $wMPARTNERID, $lagTimeFeedV2): \Walmart\Models\Supplier\US\DSVLagTime\FeedId
```
Update Lag Time
...
try {
    $result = $api->updateLagTimeBulk($feedType, $shipNode, $accept, $wMPARTNERID, $lagTimeFeedV2);
    print_r($result);
} catch (Exception $e) {
